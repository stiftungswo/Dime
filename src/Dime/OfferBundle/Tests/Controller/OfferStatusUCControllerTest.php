...
class OfferStatusUCControllerTest extends DimeTestCase
{
    ...
    public function testPostPutDeleteOfferActions()
    {
        $this->loginAs('admin');
        /* create new service */
        $response = $this->jsonRequest('POST', $this->api_prefix.'/offerstatusucs',
		    json_encode(array(
			    'text' => 'Test',
                'active' => true
		    ))
        ...
        /* modify  */
        $response = $this->jsonRequest('PUT', $this->api_prefix.'/offerstatusucs/' . $id,
		    json_encode(array(
			    'text' => 'Modified Test'
		    ))
        );
        ...
        $response = $this->jsonRequest('PUT', $this->api_prefix.'/offerstatusucs/' . ($id+100),
	    json_encode(array(
