...
class OfferPositionControllerTest extends DimeTestCase
{
    ...
    {
        $this->assertEquals(302, $this->jsonRequest('GET', $this->api_prefix.'/offerpositions')->getStatusCode());
        $this->loginAs('admin');
    ...
    public function testPostPutDeleteOfferActions()
    {
        ...
        $response = $this->jsonRequest(
            'POST',
            $this->api_prefix.'/offerpositions',
            json_encode(array(
        ...
        $response = $this->jsonRequest(
            'PUT',
            $this->api_prefix.'/offerpositions/'.$id,
            json_encode(array(
                'amount' => 45
            ))
        );
        ...
        $response = $this->jsonRequest(
            'PUT',
            $this->api_prefix.'/offerpositions/'.($id+100),
            json_encode(array(
                'amount' => 77
            ))
        );
