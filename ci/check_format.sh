...
if [ $code1 -ne $code2 ]; then
  echo "PHP backend is not properly formatted. Please reformat and commit again. See #TODO"
  exit 1;
fi
...
if [ $? -ne 0 ]; then
  echo "Dart frontend is not properly formatted. Please reformat and commit again. See #TODO"
  exit 1;
fi
